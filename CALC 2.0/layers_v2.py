...
def rand_warp(images, out_size, max_warp=0.5, name='rand_hom'):
    ...
    pts_orig = tf.concat([tf.reshape(px, [-1, 1]), 
                          tf.reshape(py, [-1, 1])], axis=-1)
    x = pts_orig[:,0:1]
    y = pts_orig[:,1:2]

    rx1 = tf.random.uniform([2, 1], -1., -1.+ max_warp)
    rx2 = tf.random.uniform([2, 1], 1.- max_warp, 1.)
    rx = tf.concat([rx1, rx2], axis=0)

    ry1 = tf.random.uniform([2, 1], -1., -1.+max_warp)
    ry2 = tf.random.uniform([2, 1], 1.-max_warp, 1.)
    ry = tf.reshape(tf.concat([ry1, ry2], axis=-1), [4, 1])
    pts_warp = tf.concat([rx, ry], axis=-1)
...
def hom_warp(images, out_size, h, name='hom_warp'):
    ...
    def _interpolate(im, x, y, out_size):
        # constants
        height = tf.shape(im)[0]
        width = tf.shape(im)[1]
        channels = tf.shape(im)[2]
        ...
        dim2 = width
        dim1 = width*height
        base = _repeat(tf.range(1)*dim1, out_height*out_width)
    ...
    def _transform(images, out_size):
        ...
        num_channels = images.get_shape()[-1]
        ...
        x, y = tf.meshgrid(x, y)
        grid = tf.concat([tf.expand_dims(tf.reshape(x,[-1]),0), tf.expand_dims(tf.reshape(y,[-1]),0)], 0)
        grid_hom = tf.concat([grid, tf.ones([1, tf.shape(grid)[-1]])], axis=0)
        ...
        input_transformed = _interpolate(images, x_s_flat, y_s_flat, out_size)
        output = tf.reshape(input_transformed, tf.stack([out_height, out_width, num_channels]))
        return output
...
def estimate_hom(src, dst):
    ...
    x = dst[:,0:1]
    y = dst[:,1:2]
    num_pts = tf.shape(src)[0]
    _0 = tf.zeros([num_pts, 3])
    _1 = tf.ones([num_pts, 1])
    ...
    A = tf.reshape(A, [2*num_pts, 9])
    _, _, V = tf.linalg.svd(A, full_matrices=True)
    return tf.reshape(V[:,-1], [3, 3])
