...
from tensorflow.python.client import device_lib  # Use to list devices available in the local process.
...
def hard_neg_mine(descr):
    ...
    tlive = tf.tile(descr, [n, 1])  # [l0, l1, l2..., l0, l1, l2...]

    tmem = tf.reshape(tf.tile(tf.expand_dims(descr, 1), [1, n, 1]),
    ...
    ids = tf.argmax(input=sim_sq, axis=-1, output_type=tf.int32)

    # I guess just contiguously index it?
    row_inds = tf.range(0, n, dtype=tf.int32) * (n - 1)
...
def standard_model_fn(func, steps, run_config, optimizer_fn=None, eval_steps=32, model_dir='model'):
    ...
    def fn(features, labels, mode, params):
        ...
        if is_training:
            ...
            if optimizer_fn is None:
                # optimizer = tf.keras.optimizers.Adam(learning_rate=params['learning_rate'], clipnorm=5)
                optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=params['learning_rate'])
            else:
                optimizer = optimizer_fn

            train_op = optimizer.minimize(ret["loss"], tf.compat.v1.train.get_or_create_global_step())
...
def num_gpus():
    """
    List the devices available (especially GPUs) in the local process.
    """
...
def train_and_eval(model_dir,
    ...
    Args:
    model_dir: The output directory where model parameters, graphs, etc are saved.
    steps: Training steps.
