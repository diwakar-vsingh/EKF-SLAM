...
parser.add_argument("-o", "--output_dir", type=str, metavar='', 
                    default="mnt/coco/calc_tfrecords/", 
                    help="path to store tfrecords of training input images")
parser.add_argument("-c", "--coco_root", type=str, metavar='', 
                    default="mnt/coco", help="path to train image dataset")
parser.add_argument("-n", "--num_files", type=int, metavar='', 
                    default=100, help="Number of files to write for train dataset. More files=better randomness")
parser.add_argument("-d", "--debug", default=True, action='store_false', 
                    help='Omitting the switch sets args.bool to True, otherwise it is False \
...
def tfgenerate():
    # tfrecord files for train dataset
    train_writers = []
    for ii in range(args.num_files):
        train_writers.append(None if args.debug else \
                                 tf.io.TFRecordWriter(args.output_dir + "train_data%d.tfrecord" % ii))

    # tfrecord files for val dataset
    ...
    for split, writer in [('train', train_writers), ('val', val_writer)]:
        ...
        for image_id in dataset.image_ids:
            ...
            if args.debug:
                ...
                if split == 'val':
                    features_['cl_live'] = _bytes_feature(tf.compat.as_bytes(cl_live.tostring())),
                    features_['cl_mem'] = _bytes_feature(tf.compat.as_bytes(cl_mem.tostring())),
                ...
                if split == 'val':
                    ...
                    writer[np.random.randint(0, args.num_files)].write(example.SerializeToString())
