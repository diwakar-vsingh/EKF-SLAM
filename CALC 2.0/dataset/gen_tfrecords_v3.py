...
def serialize_example(split, image, mask):
    ...
    if split == 'train':
        ...
        # feature = {
        #     'img': _bytes_feature(tf.io.serialize_tensor(image)), 
        #     'label': _bytes_feature(tf.compat.as_bytes(mask.astype(np.uint8).tostring()))
        # } 
...
def tfgenerate_train():
    ...
    # Iterate through each image in train dataset to generate tfrecord
    count = 1
    for image_id in dataset.image_ids[:2]:
        print("Working on training sample %d" % image_id)
...
def tfgenerate_val():
    ...
    # Iterate through each image in val dataset to generate tfrecord
    split = 'val'
    for image_id in range(1, 101):
        print("Working on validation sample %d" % image_id)
        cl_live = cv.cvtColor(cv.resize(
            cv.imread("CampusLoopDataset/live/Image%s.jpg" % (str(image_id).zfill(3))),
            (vw, vh), interpolation=cv.INTER_AREA), cv.COLOR_BGR2RGB)
        cl_mem = cv.cvtColor(cv.resize(
            cv.imread("CampusLoopDataset/memory/Image%s.jpg" % (str(image_id).zfill(3))),
            (vw, vh), interpolation=cv.INTER_AREA), cv.COLOR_BGR2RGB)
...
if __name__ == "__main__":
    ...
    # Generate tfrecord for validation dataset
    tfgenerate_val()

    # Generate tfrecord for training dataset
    # tfgenerate_train()
