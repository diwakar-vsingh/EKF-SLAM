...
if __name__ == '__main__':
    tf.compat.v1.flags.DEFINE_string("output_dir", "mnt/f3be6b3c-80bb-492a-98bf-4d0d674a51d6/coco/calc_tfrecords/", "")
    tf.compat.v1.flags.DEFINE_string("coco_root", "mnt/f3be6b3c-80bb-492a-98bf-4d0d674a51d6/coco/", "")
    tf.compat.v1.flags.DEFINE_integer("num_files", 100,
                                      "Num files to write for train dataset. More files=better randomness")
    tf.compat.v1.flags.DEFINE_boolean("debug", False, "")
...
def generate():
    ...
    for ii in range(FLAGS.num_files):
        ...
        # else:
        #     train_writers.append(tf.io.TFRecordWriter(FLAGS.output_dir + "train_data%d.tfrecord" % ii))    
        train_writers.append(tf.io.TFRecordWriter(FLAGS.output_dir + "train_data%d.tfrecord" % ii))    
        # train_writers.append(None if FLAGS.debug else tf.io.TFRecordWriter(FLAGS.output_dir + "train_data%d.tfrecord" % ii))

    # if FLAGS.debug:
    #     val_writer = None
    # else:
    #     val_writer = tf.io.TFRecordWriter(FLAGS.output_dir + "validation_data.tfrecord")
    val_writer = tf.io.TFRecordWriter(FLAGS.output_dir + "validation_data.tfrecord")
    ...
    for split, writer in [('train', train_writers), ('val', val_writer)]:
        ...
        for image_id in dataset.image_ids:
            ...
            # if FLAGS.debug == True:
                # print("Showing Images with mask")
            ...
            features_ = { 'img': bytes_feature(tf.compat.as_bytes(image.tostring())), 'label': bytes_feature(tf.compat.as_bytes(mask.astype(np.uint8).tostring()))}
