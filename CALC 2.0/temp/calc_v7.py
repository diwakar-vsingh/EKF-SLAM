...
def VSS(inputShape, alpha=1.0):
    ...
    model = Model(encoder_input, [rec, seg], name="Autoencoder")
...
def model_fn(features, labels, mode, hparams):
    ...
    x = tf.image.random_crop(im_l, size=[sz, vh, vw, 3 + N_CLASSES])
    ...
    ##################################### Latent Variables #####################################################
    mu = vss.get_layer(name="mu").output
    log_sig_sq = vss.get_layer(name="log_sig_sq").output
    z = vss.get_layer(name="z").output

    sh = mu.get_shape().as_list()
    c_centers = tf.Variable(tf.random.normal([1, sh[1], sh[2], sh[3]]), 
                            name='offset', 
                            trainable=True)
    ...
    descr = K.l2_normalize(decr, axis=-1, name='descriptor')
    ...
    input_shape = im_warp.get_shape().as_list()
    input_shape = (input_shape[1], input_shape[2], input_shape[3])

    descriptor_model = Model(inputs=vss.input, outputs=features_list)
    mu = descriptor_model(im_warp, is_training) 
    ...
    # Normalize the entire descriptor to allow for cosine similarilty calculation by inner product
    # descr = tf.math.l2_normalize(descr, axis=-1, name='descriptor')
    descr_p = K.l2_normalize(descr_p, axis=-1, name='descriptor')
    ...
    seg = vss.get_layer(name="seg").output   
    if is_training:
    ...
    rec = vss.get_layer(name="rec").output
    ...
    # vss.compile(optimizer=optimizer, loss=loss)

    # Define metrics
    # vss.add_metric(tf.keras.metrics.Mean(name='Test_Error')(loss))
    # vss.add_metric(tf.keras.metrics.Mean(name='Seg_Error')(segloss))
    # vss.add_metric(tf.keras.metrics.Mean(name='Rec_Error')(recloss))
    # vss.add_metric(tf.keras.metrics.Mean(name='KLD_Error')(kld))
    # vss.add_metric(tf.keras.metrics.Mean(name='Sim_Error')(simloss))
...
def main(argv):
    ...
    if args.mode == 'train':
        ...
        utils.train_and_eval(
            ...
            input_fn=make_input_fn,   # create_input_fn(split, batch_size)
            hparams=hparams,
        )
